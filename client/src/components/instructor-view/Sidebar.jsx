...
import { ChevronLeft, ChevronRight, Home, BookOpen, Users, BarChart2, LogOut } from "lucide-react";
...
function Sidebar() {
  ...
  const navItems = [
    { title: "Dashboard", to: "/instructor", icon: Home, color: "from-orange-500 to-orange-400" },
    { title: "Courses", to: "/instructor/courses", icon: BookOpen, color: "from-orange-500 to-orange-400" },
    { title: "Students", to: "/instructor/add-students", icon: Users, color: "from-orange-500 to-orange-400" },
    { title: "Analytics", to: "/instructor/analytics", icon: BarChart2, color: "from-orange-500 to-orange-400" },
  ];
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <div className={cn(
        "fixed top-16 left-0 flex flex-col h-[calc(100vh-4rem)] bg-white text-gray-700 transition-all duration-300 border-r border-gray-100 z-40",
        collapsed ? "w-20" : "w-64"
      )}>
        {/* Profile Section */}
          ...
          <div className={cn(
              ...
              <Avatar className="h-10 w-10 border-2 border-orange-500">
                {auth.user?.profileImage ? (
                  ...
                  <AvatarFallback className="bg-gray-100 text-gray-700 text-xl font-bold">
                    {auth.user?.userName?.charAt(0).toUpperCase() || "I"}
                  </AvatarFallback>
                )}
              </Avatar>
            </div>
            ...
            {!collapsed && (
              <div className="flex-1">
                <p className="font-bold text-sm text-gray-800 uppercase">{auth.user?.userName || "INSTRUCTOR"}</p>
                <div className="flex items-center justify-between">
                  <p className="text-xs text-gray-500">{auth.user?.role || "Instructor"}</p>
                  <Button
                    variant="ghost"
                    size="icon"
