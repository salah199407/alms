...
import { ChevronLeft, ChevronRight, Home, Users, Bell, Settings, LogOut, Building } from "lucide-react";
...
function Sidebar() {
  ...
  const navItems = [
    { title: "Dashboard", to: "/super-admin", icon: Home, color: "from-orange-500 to-orange-400" },
    { title: "Organizations", to: "/super-admin/organizations", icon: Building, color: "from-orange-500 to-orange-400" },
    { title: "Users", to: "/super-admin/users", icon: Users, color: "from-orange-500 to-orange-400" },
    { title: "Notifications", to: "/super-admin/notifications", icon: Bell, color: "from-orange-500 to-orange-400" },
  ];

  const settingsItems = [
    { title: "Profile Settings", to: "/profile-settings", icon: Settings, color: "from-orange-500 to-orange-400" },
  ];

  const handleLogout = () => {
    resetCredentials();
    sessionStorage.clear();
    navigate("/auth"); // ✅ rediriger après déconnexion
  };
          ...
          <div className={cn(
            ...
            {!collapsed && (
              <div className="flex-1">
                <p className="font-bold text-sm text-gray-800 uppercase">{auth.user?.userName || "SUPER ADMIN"}</p>
                <div className="flex items-center justify-between">
                  <p className="text-xs text-gray-500">{auth.user?.role || "Administrator"}</p>
                  <Button
                    variant="ghost"
                    size="icon"
